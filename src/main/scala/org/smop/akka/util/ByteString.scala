...
/**
 * A mutable builder for efficiently creating a [[util.ByteString]].
 *
...
final class ByteStringBuilder extends Builder[Byte, ByteString] {
  ...
  def putBytes(array: Array[Byte], start: Int, len: Int): this.type =
    fillArray(len) {
      case (target, targetOffset) ⇒ Array.copy(array, start, target, targetOffset, len)
    }
